...
import { LayoutDashboard, Target, Lightbulb, CalendarDays, Smartphone, ChevronDown, FolderOpen, Trophy, Presentation, PanelLeftClose, PanelLeftOpen } from "lucide-react";
...
const q3MenuItems = [
  { title: "Roadmap", url: "/q3-2026/roadmap", icon: CalendarDays },
  { title: "Roadmap Review", url: "/q3-2026/roadmap-review", icon: Presentation },
];
...
export function AppSidebar() {
  ...
  const isQ3Route = location.pathname.startsWith("/q3-2026");
  ...
  const [isOpenQ3, setIsOpenQ3] = useState(isQ3Route);
        ...
        {collapsed ? (
          <div className="flex justify-center">
            <Button
              ...
              className="h-9 w-9 text-sidebar-foreground/80 hover:bg-white/10 hover:text-sidebar-foreground"
              title="Expandir menú"
          ...
          <div className="flex items-center gap-3">
            ...
            <Button
              ...
              size="icon"
              onClick={toggleSidebar}
              className="ml-auto h-8 w-8 text-sidebar-foreground/80 hover:bg-white/10 hover:text-sidebar-foreground"
              title="Colapsar menú"
        ...
        <SidebarGroup className={collapsed ? "px-1" : ""}>
          {collapsed ? (
            ...
            <>
              ...
              <Collapsible open={isOpenQ3} onOpenChange={setIsOpenQ3}>
                <CollapsibleTrigger className="group flex w-full items-center gap-2 rounded-lg px-3 py-2.5 text-sm font-medium text-sidebar-foreground/90 transition-all duration-300 ease-in-out hover:bg-white/10 hover:shadow-md hover:translate-x-0.5">
                  <FolderOpen className="h-5 w-5 text-alegra-orange transition-transform duration-300 group-hover:scale-110" />
                  <span className="flex-1 text-left">Q3 2026</span>
                  <ChevronDown
                    className="h-4 w-4 transition-transform duration-300 ease-in-out"
                    style={{ transform: isOpenQ3 ? "rotate(180deg)" : "rotate(0deg)" }}
                  />
                </CollapsibleTrigger>
                <CollapsibleContent className="ml-4 mt-1 border-l-2 border-white/20 pl-3">
                  <SidebarGroupContent>
                    <SidebarMenu>
                      {q3MenuItems.map((item) => (
                        <SidebarMenuItem key={item.title}>
                          <SidebarMenuButton asChild>
                            <NavLink
                              to={item.url}
